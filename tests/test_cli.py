...
def test_publish_gcs_command_parses_expected_current_version():
    expected_version = f"sha256-{'c' * 64}"
    ...
    assert args.bundle_name == "manifest-gcs"
    assert args.expected_current_version == expected_version
...
def test_publish_gcs_command_rejects_non_gcs_bundle(tmp_path):
    config = [
        {
            "name": "manifest-local",
            "classpath": "airflow_manifest_bundle.local.ManifestLocalDagBundle",
    ...
    ), pytest.raises(SystemExit, match="not configured as a ManifestGCSDagBundle"):
        cli.main(["publish-gcs", "manifest-local"])
...
def test_publish_local_command_rejects_automatic_local_bundle(tmp_path):
    ...
    ), pytest.raises(SystemExit, match="source_path configured for automatic publication"):
